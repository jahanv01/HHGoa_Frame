...
const ILLUSTRATED_BANDS = {
  radio: { bg: GREEN, text1: CREAM, text2: PINK },
  postcard: { bg: CREAM, text1: GREEN, text2: PINK },
  map: { bg: CREAM, text1: GREEN, text2: PINK },
};
...
async function renderIllustrated(name, photo) {
  ...
  const band = ILLUSTRATED_BANDS[name];
  const canvas = createCanvas(W, W + BAND_H);
  ...
  ctx.fillRect(0, W, W, BAND_H);
  ctx.textAlign = 'center';
  ctx.fillStyle = band.text1;
  ctx.font = '500 14px sans-serif';
  ctx.fillText(TAGLINE, W / 2, W + 28);
  ctx.font = '600 22px sans-serif';
  ctx.fillText(DATE_LINE, W / 2, W + 56);
  ctx.fillStyle = band.text2;
  ctx.font = '700 22px sans-serif';
  ctx.fillText(HASHTAG, W / 2, W + 82);
